...
void ObstacolDublu::verificaInel(Player& player, float raza, float rotatie, const std::vector<Culoare>& culori) {
...
void ObstacolDouaElice::verificaElice(Player& p, sf::Vector2f centru, float rotatie, const std::vector<Culoare>& culori) {
...
SchimbatorCuloare::SchimbatorCuloare(float y, TipCuloare culoareDeEvitat)
    ...
    do { nouaCuloare = static_cast<TipCuloare>(rand() % 4); } while (nouaCuloare == culoareDeEvitat);
    this->culoare_ = Culoare(nouaCuloare);
}
...
bool SchimbatorCuloare::interactioneaza(Player& player) {
    if (std::abs(player.getY() - y_) < 20.0f) {
        player.setCuloare(this->culoare_);
...
void SchimbatorCuloare::draw(sf::RenderWindow& window) const {
    ...
    shape.setFillColor(culoare_.getSFMLColor());
